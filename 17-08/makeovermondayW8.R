...
mydf <- function(yy){
  pot.df <- pot %>% filter(Year == yy) %>% arrange(Country) %>% 
    mutate(Land=Country, color="red") %>% select(Land,Year,`Yield.(100.kg/ha)`,Area.in.1000ha, color)
...
myplot <- function(i){
  g <- tween_pot %>% filter(i == .frame) %>% 
    ggplot(aes(y=`Yield.(100.kg/ha)`, x=Area.in.1000ha)) + 
    geom_point() + 
...
tween_pot<-tween_states(plotingList,tweenlength=2,statelength=2, ease='cubic-in-out', nframes=50)
